...
<%  try{%> <div class="header">Marked Letters</div><br><br>
			...
			<form action="emp-mark-action.jsp" method="post">
			...
			con = DriverManager.getConnection("jdbc:ucanaccess://G:\\Ecilipse\\DAK Management\\WebContent\\database.accdb");
			st = con.createStatement();
			rs = st.executeQuery("select sno,lno,sub,date1,frm,msg,file from smain where gto='"+name+"'");
			...
			while(rs.next()){%>
					...
					<td>NA</td>
					<td>Sending</td>
					<td><input type="radio" value="create" name="sub-type"><br>Re-Create<br><input type="radio" value="del" name="sub-type"><br>Delete<br>
			...
			if(sr.equals("1")){
				try{	//out.println("hello"+sr);
						...
						rs = st.executeQuery("select sno,lno,sub,date1,msg,file, rmsg, rfile, rdate from smain where gresp='"+name+"'");
						...
						<%	session.setAttribute("filename1", rs.getString(8)); %>
						<%	session.setAttribute("filename", rs.getString(6)); %>
						<td>Group-<%=rs.getString(5)%><br>
						<td><a href="DownloadServlet"><%=rs.getString(6)%></a></td>
						<td><%=rs.getString(7)%><br>
						<td><a href="DownloadServlet1"><%=rs.getString(8)%></a></td>
